...
- 新增 `@dsh-reliability/core`：`loadCases` / `screenAnswer` / `createEvidenceStore` / `runReliability` 稳定接口，以及 `MockAdapter`、`RestApiAdapter`、`DriverAdapter`（真实 JSONL DSH 驱动，自包含，驱动脚本路径由调用方注入）。
...
- 评分规则沿用已验收的保守初筛语义（事实边界、版本冲突、未知信息），附一致性守门测试，保证无规则分叉。
...
- 已知限制：真实 DSH 驱动脚本与容器不在发布包内，需外部提供；npm 包尚未正式发布。
